...
import { IMAGE_BASE_PATH, POST_REVIEW_API, POST_CANCEL_ORDER, GET_RETURNS_REASONS, POST_RETURN_PRODUCT_API, POST_ORDER_DETAILS } from "../../config/ApiConfig";
...
function OrderDetails({ navigation, route }) {
    ...
    const _returnDateCal = (date_purchased, returnable_days) => {
        ...
        var orderDateStr = Moment(date_purchased)
            .utc();
    ...
    const _submitReturnComment = () => {
        if (returnReason == "") {
            ...
            PostApiFetch(POST_RETURN_PRODUCT_API, formData)
                .then(([status, response]) => {
                    if (status == 200) {
                        // console.log("response==>", response)
                        modalClose()
            ...
            <View style={pageContainerStyle}>
                    ...
                    <View style={styles.orderDetailsHeadSec}>
                        ...
                        <View style={styles.orderDetailsSec}>
                            <View style={{ flex: 1 }}>
                                ...
                                <Text style={styles.orderTextLeft}>{orderDetails.invoice_number}</Text>
                            </View>
                            <View style={{ flex: 1 }}>
                                <Text style={styles.orderTopHeading}>Placed On</Text>
                                <Text style={styles.orderText}>{orderDetails.date_purchased}</Text>
                            </View>
                        </View>
                        <View style={styles.orderDetailsSec}>
                            <View style={{ flex: 1 }}>
                                <Text style={styles.orderTopHeading}>Total Amount</Text>
                                <Text style={styles.orderTextLeft}>{orderDetails.order_price}</Text>
                            </View>
                            <View style={{ flex: 1 }}>
                                <Text style={styles.orderTopHeading}>Payment Type</Text>
                                <Text style={styles.orderText}>{orderDetails.payment_method}</Text>
                            </View>
                        </View>
                        <View style={styles.orderDetailsSec}>
                            <View style={{ flex: 1 }}>
                                <Text style={styles.orderTopHeading}>Customer Name</Text>
                                <Text style={styles.orderTextLeft}>{orderDetails.customers_name}</Text>
                            </View>
                            <View style={{ flex: 1 }}>
                                <Text style={styles.orderTopHeading}>Order Status</Text>
                                <Text style={styles.orderTextLeft}>{orderDetails.orders_status}</Text>
                            </View>
                        </View>
                            ...
                            {orderDetails.orders_status != "Return" ?
                                <>
                                    {orderDetails.statusess[0].cancellable == 1 &&
                                        <TouchableOpacity style={activeButton.cancelButton} onPress={() => {
                                            // console.log("Cancel")
                                            _cancelOrder()
                                        }}>
                            ...
                            <View style={styles.contactUsSec} key={key}>
                                ...
                                <View style={styles.productDetailsSec}>
                                    ...
                                    <View style={styles.productQtySec}>
                                        ...
                                        <Text style={styles.productQty}> {item2.products_quantity}</Text>
                                    </View>
                                    <View style={styles.productQtySec}>
                                        <Text style={styles.productQtyText}>{item2.attributes[0].products_options} : </Text>
                                        <Text style={styles.productQty}> {item2.attributes[0].products_options_values}</Text>
                                    </View>
                                    <Text style={styles.productPrice}>Price : ₹{item2.products_price}</Text>

                                    <View style={styles.extraSec}>
                                        {orderDetails.orders_status == 'Completed' &&
                                            <TouchableOpacity style={activeButton.reviewButton} onPress={() => {
                                        ...
                                        {orderDetails.orders_status == "Completed" ?
                                            <>
                                                {item2.returnable_days != 0 && _returnDateCal(orderDetails.date_purchased, item2.returnable_days) && returnRequested == 0 ?
                                                    <TouchableOpacity style={activeButton.reviewButton} onPress={() => {
                                                        modalShow()
                                                    }}>
                                                ...
                                                {returnRequested == 1 ?
                                                    <Text style={styles.returnProduct}>Return Request Submited</Text>
                                                    :
                                                    <></>
                                                }
                                            </>
                        ...
                        {orderDetails.allGift.map((item3, key3) => (
                            <TouchableOpacity style={styles.contactUsSec} key={key3}>
                                ...
                                <View style={styles.productDetailsSec}>
                                    <Text style={styles.productName}>{item3.order_gift_title}</Text>
                                </View>
                            </TouchableOpacity>
                        ))}
                ...
                >
                    <ScrollView>
                        ...
                        <View>
                            <Text style={{ textAlign: "center", color: '#EC1F25' }}>{fieldMessage}</Text>
                        </View>
                        <View>
                            ...
                            <TouchableOpacity style={[activeButton.reviewButton, { alignSelf: 'center' }]} onPress={() => {
                                _submitReview();
                            }}>
