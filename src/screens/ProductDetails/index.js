...
import { pageContainerStyle, pageHeader, inputLevel, inputBottomLevel, textInput, inputContainer, activeButton, fontSize, passiveButton } from "../../common/values/BKStyles";
import { BKColor } from "../../common/values/BKColor";
import { FlatListSlider } from "react-native-flatlist-slider";
import { GET_PRODUCT_DETAILS_API, UPDATE_CART_QUANTITY } from "../../config/ApiConfig";
...
function ProductDetails({ navigation, route }) {
    ...
    const [productsAttributes, setProductsAttributes] = useState(route.params.productsAttributes)
    const [preUrl, setPreUrl] = useState(route.params.preUrl)
    // console.log('preUrl ===============> ',route.params)
    const [productImages, setProductImages] = useState([])
    const [isLoading, setIsLoading] = useState(true);
    const [productsAttributesChange, setProductsAttributesChange] = useState(false);
    ...
    const _getProductDetails = (androidId) => {
        ...
        let params = "";
        if (userData != null) {
            params = '?products_slug=' + productsSlug + "&customers_id=" + userData.id + "&session_id=";
        } else {
            params = '?products_slug=' + productsSlug + "&customers_id=&session_id=" + androidId;
        }
        if (preUrl == 'SingleProduct') {
            for (let item of productsAttributes) {
                params = params + '&' + item.option.product_option_slug + '=' + item.values1[0].value
            }
        } else if (preUrl == 'Cart') {
            for (let item of productsAttributes) {
                params = params + '&' + item.product_option_slug + '=' + item.attribute_value
            }
        }
    ...
    const _buyNow = () => {
        if (userData != null) {
            addToCart(productDetails.products_id, quantity, userData.id, '', 1, productDetails.prod_attributeids).then(([status, response]) => {
                ...
                <ScrollView style={{ marginVertical: hp('3%'), }}>
                    ...
                    <View style={styles.productNameAttrSec}>
                        <View style={styles.cartWishlistIcon}>
                            <Text style={styles.productName}>{productDetails.products_name}</Text>
                            {userData != null ?
                                <TouchableOpacity onPress={() => {
                                    addToWishlist(userData.id, productDetails.products_id, productDetails.products_attributes_prices_id).then(([wishlistData]) => {
                                    ...
                                    {wishlistData != null && wishlistData.some(value => value.products_id == productDetails.products_id && value.products_attributes_prices_id == productDetails.products_attributes_prices_id) ?
                        ...
                        {productDetails.attributes.map((item, key) => (
                            <View key={key} style={{ flexDirection: 'row', alignItems: 'center', marginTop: wp('2%') }}>
                                ...
                                {item.values.map((item2, key2) => (
                                    ...
                                    }} style={item.values1[0].products_attributes_id == item2.products_attributes_id ? styles.productAttrItemActive : styles.productAttrItem}>
                                        <Text style={item.values1[0].products_attributes_id == item2.products_attributes_id && { color: BKColor.white }}>{item2.value}</Text>
                                    </TouchableOpacity>
                                ))}
                            </View>
                        ))}
                    ...
                    <View style={styles.productDecSec}>
                        ...
                        <Text style={styles.productDec}>{productDetails.products_special_instructions != null ? (productDetails.products_special_instructions).replace(regex, '') : ''}</Text>
                        <Text style={styles.productDec}>{productDetails.products_description != null ? ((productDetails.products_description).replace(regex, '')).replace(secondRegEx, '') : ''}</Text>
                        ...
                        {productDetails.defaultStock > 0 ?
                            <>
                                {productDetails.isCartPresent == 1 ?
                                    <View style={[passiveButton.button, { padding: wp('3%'), borderColor: BKColor.textColor2 }]}>
                                        <Text style={passiveButton.text}>Added</Text>
                                    </View>
                                    :
                                    <>
                                        <View style={styles.addToCartBtn}>
                                            <TouchableOpacity style={[passiveButton.button, { padding: wp('3%'), }]} onPress={() => {
                                                if (productDetails.defaultStock > 0) {
                                                    userData != null ?
                                                        ...
                                                        :
                                                        addToCart(productDetails.products_id, quantity, '', androidId, '', productDetails.prod_attributeids).then(([status, response]) => {
                                                            setChangeInCarts(true)
                                                            if (status == 200) {
                                                                if (response.status === true) {
                                                                    dispatch({ type: 'setCartData', payload: response.cart })
                                                ...
                                                <AntDesign name="shoppingcart" style={passiveButton.icon} />
                                            </TouchableOpacity>
                                        </View>
