...
import { GET_MY_ADDRESS_API, GET_CART_API, POST_ADD_ORDER, GET_PAYMENT_METHODS, POST_GENERATE_ORDER_ID } from "../../config/ApiConfig";
...
function Checkout({ navigation }) {
    ...
    const [sameAsBilling, setSameAsBilling] = useState(selectedShippingAddress != null ? 0 : 1);
    ...
    const _calculateAmounts = async (cart, shipping_detail, couponDiscountPercent, usedLoyaltyPoint) => {
    ...
    const _placeOrderCashOnDelivery = (razorpay_payment_id = '') => {
        if (userBillingAddress == null) {
            ...
            formData.append('customers_email', sameAsBilling === 1 ? userBillingAddress.entry_email : selectedShippingAddress.entry_email);
            formData.append('orderBillingAddressBookId', userBillingAddress.address_book_id);
            formData.append('address_id_hidden', sameAsBilling === 1 ? '' : selectedShippingAddress.address_book_id);
            ...
            if (couponDetails != null) {
                ...
                formData.append('coupon_discount_percent', couponDetails.coupon_discount_percent);
            } else {
            ...
            <ScrollView style={pageContainerStyle}>
                    ...
                    <TouchableOpacity style={styles.regContainer.item} onPress={() => navigation.navigate('MyAddress')}>
                        <View style={styles.itemOuter}>
                            <View style={styles.textOuter}>
                                <Ionicons name="location-outline" color='#000000' size={fontSize.h2} />
                                <Text style={styles.regContainer.text1}>Billing Addresses</Text>
                            </View>
                    ...
                    {userBillingAddress != null ?
                        <View style={styles.customerDetailsSec}>
                            <View style={styles.checkBoxIcon}>
                                <Fontisto name="checkbox-active" color='#000000' size={fontSize.h3} />
                            </View>
                            <View style={styles.customerDetails}>
                                <Text style={styles.regContainer.text2}>{userBillingAddress.entry_firstname}</Text>
                                <Text style={styles.contactUsText}>{userBillingAddress.entry_street_address}, {userBillingAddress.entry_city}, {userBillingAddress.states_name}, {userBillingAddress.districts_name}, {userBillingAddress.pincodes_val}</Text>
                                <View style={styles.contactUsSec}>
                                    <Text style={styles.contactUsLabel}>Mobile Number :</Text>
                                    <Text style={styles.contactUsText}> {userBillingAddress.entry_phone}</Text>
                                </View>
                                <View style={styles.contactUsSec}>
                                    <Text style={styles.contactUsLabel}>Email :</Text>
                                    <Text style={styles.contactUsText}> {userBillingAddress.entry_email}</Text>
                                </View>
                            </View>
                        </View>
                        :
                        <Text style={styles.contactUsLabel}>No billing address added</Text>
                    }
                ...
                <View style={styles.regContainer}>
                    ...
                    <TouchableOpacity style={styles.regContainer.item} onPress={() => navigation.navigate('MyAddress')}>
                        <View style={styles.itemOuter}>
                            <View style={styles.textOuter}>
                                <Ionicons name="location-outline" color='#000000' size={fontSize.h2} />
                                <Text style={styles.regContainer.text1}>Shipping Addresses</Text>
                            </View>
                            <Entypo name="chevron-thin-right" color='#000000' size={fontSize.h2} />
                        </View>
                    </TouchableOpacity>
                    <View style={styles.customerDetailsSec}>
                        {sameAsBilling === 1 ?
                            <TouchableOpacity style={styles.checkBoxIcon} onPress={() => {
                                setSameAsBilling(0)
                            }}>
                                <Fontisto name="checkbox-active" color='#000000' size={fontSize.h3} />
                            </TouchableOpacity>
                            ...
                            <TouchableOpacity style={styles.checkBoxIcon} onPress={() => {
                                setSameAsBilling(1)
                                dispatch({ type: 'setSelectedShippingAddressData', payload: null });

                            }}>
                                <Fontisto name="checkbox-passive" color='#000000' size={fontSize.h3} />
                            </TouchableOpacity>
                ...
                <View style={styles.regContainer}>
                    {userShippingAddressList.length > 0 ?
                        <View>
                            {
                                userShippingAddressList.map((item, key) => (
                                    ...
                                    <View style={styles.customerDetailsSec} key={key}>
                                        ...
                                        {selectedShippingAddress != null && item.address_book_id == selectedShippingAddress.address_book_id ?
                                        ...
                                        <View style={styles.customerDetails}>
                                            <Text style={styles.regContainer.text2}>{item.entry_firstname}</Text>
                                            <Text style={styles.contactUsText}>{item.entry_street_address}, {item.entry_city}, {item.states_name}, {item.districts_name}, {item.pincodes_val}</Text>
                                            <View style={styles.contactUsSec}>
                                                <Text style={styles.contactUsLabel}>Mobile Number :</Text>
                                                <Text style={styles.contactUsText}> {item.entry_phone}</Text>
                                            </View>
                                            <View style={styles.contactUsSec}>
                                                <Text style={styles.contactUsLabel}>Email :</Text>
                                                <Text style={styles.contactUsText}> {item.entry_email}</Text>
                                            </View>
                                        </View>
                                    </View>
                                ))
                        ...
                        :
                        <Text style={styles.contactUsLabel}>No shipping address added</Text>
                ...
                <View style={styles.regContainer}>
                    ...
                    <TouchableOpacity style={styles.regContainer.item} onPress={() => navigation.navigate('MyAddress')}>
                        <View style={styles.orderOuter}>
                            <View style={styles.textOuter}>
                                <Text style={styles.regContainer.orderHeading}>Your Order</Text>
                            </View>
                        </View>
                    </TouchableOpacity>
                    <View style={styles.customerDetailsSec}>
                            ...
                            {cartData.map((item, key) => (
                                <View style={styles.customerOrderSec} key={key}>
                                    <View style={styles.productSec}>
                                        <Text style={styles.regContainer.text3}>{item.products_name} x {item.customers_basket_quantity}</Text>
                                    </View>
                                    <View style={styles.totalSec}>
                                        <Text style={styles.orderTotal}>₹ {item.afterDiscountPrice * item.customers_basket_quantity}</Text>
                                    </View>
                                </View>
                            ))}
                        </>
                ...
                <View style={{ marginVertical: hp('3%') }}>
                    <View style={styles.paymentSec}>
                        {cod != null &&
                            <TouchableOpacity style={styles.paymentOuterSec} onPress={() => {
                                setPaymentMethod('cash_on_delivery')
                            }}>
                                {paymentMethod === 'cash_on_delivery' ?
                                    <MaterialIcons name="circle" color='#000000' size={fontSize.h2} />
                                    :
                                    <MaterialCommunityIcons name="circle-outline" color='#000000' size={fontSize.h2} />
                                }
                    ...
                    {paymentMethod === 'cash_on_delivery' &&
                        <TouchableOpacity style={activeButton.button} onPress={() => {
