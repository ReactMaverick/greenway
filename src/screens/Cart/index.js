...
import { GET_CART_API, IMAGE_BASE_PATH, POST_DELETE_CART_API, APPLY_COUPON, UPDATE_CART_QUANTITY } from "../../config/ApiConfig";
...
function Cart({ navigation }) {
    ...
    const { shopNowData } = useSelector((state) => state.ShopNowReducer);
    ...
    const _updateCartQuantity = (customers_basket_id, products_id, attributes_id, customers_basket_quantity) => {
        setIsLoading(true)
        const formData = new FormData();
    ...
    // min_order : 1
    // max_order : 18
    const _minusQuantity = (key, customers_basket_id, products_id, attributes_id) => {
        var customers_basket_quantity = parseInt(productCartData[key].customers_basket_quantity) - 1;
        if (customers_basket_quantity <= 0) {
            _deleteCart(customers_basket_id)
        } else {
            if (customers_basket_quantity >= productCartData[key].min_order) {
                _updateCartQuantity(customers_basket_id, products_id, attributes_id, customers_basket_quantity)
            }
        }
    }
    const _plusQuantity = (key, customers_basket_id, products_id, attributes_id) => {
        var customers_basket_quantity = parseInt(productCartData[key].customers_basket_quantity) + 1;
    ...
    if (isLoading) {
        ...
        return (
            ...
            <View style={pageContainerStyle}>
                ...
                {cartData != null ?
                    <ScrollView style={{ marginBottom: hp('17%') }} showsVerticalScrollIndicator={false} >
                            ...
                            <View style={styles.cartDetailsSec} key={key}>
                                ...
                                <View style={{ width: wp('68%') }}>
                                    <View style={styles.cartDecSec}>
                                        ...
                                        <TouchableOpacity onPress={() => _deleteCart(item.customers_basket_id)}>
                                    ...
                                    <View style={styles.cartDecSec}>
                                        <View style={styles.cartQtySec}>
                                            ...
                                            <Text style={styles.cartQty}>{item.customers_basket_quantity}</Text>
                                            <TouchableOpacity onPress={() => {
                                        ...
                                        <Text style={styles.cartPrice}>Rs{item.afterDiscountPrice}</Text>
                                    </View>
                                </View>
                            </View>
                                    ...
                                    {giftArea.map((item2, key2) => (
                                        <>
                                            <View style={styles.giftsSec} key={key2}>
                                                ...
                                                <View style={{ flexDirection: 'column', alignItems: "center", width: wp('60%') }}>
                                                    <Text style={styles.giftsItemText}>{item2.gift_title}</Text>
                                                    <View style={{ flexDirection: 'row', alignItems: "center", marginTop: hp('1%') }}>
                                                        <Text style={styles.giftsItemPriceText}>₹ {item2.gift_price}</Text>
                        ...
                        <View style={{ paddingVertical: hp('3%') }}>
                            ...
                            <View style={inputContainer}>
                                <View style={styles.couponInputSec}>
                                    ...
                                    <TouchableOpacity style={styles.couponApplyBtn} onPress={() => {
                        ...
                        <View style={{ borderBottomWidth: 2, borderStyle: "dashed", borderBottomColor: '#dddddd', paddingBottom: hp('1%') }}>
                            ...
                            {giftArea.length > 0 ?
                                <View style={styles.cartItemSec}>
                                    ...
                                    <View style={{ flexDirection: "row" }}>
                                        <Text style={styles.totalItem}>free gift with this order: </Text>
                                        <Text style={{ color: '#EC1F25', fontWeight: '700' }}>₹ 0</Text>
                                    </View>
                            ...
                            <View style={styles.cartItemSec}>
                                ...
                                <Text style={styles.totalItem}>Flat Rate: ₹ {deliveryCharges}</Text>
                            </View>
                            <View style={styles.cartItemSec}>
                                ...
                                {couponDetails != null ?
                                    <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center' }} onPress={() => {
                                        ...
                                        <Entypo name="circle-with-cross" style={[styles.totalItem, { marginRight: wp('1%'), fontSize: wp('5%') }]} />
                                        <Text style={styles.totalItem}>{couponDetails.coupon[0].code}: ₹ {couponDiscount}</Text>
                                    </TouchableOpacity>
                                    :
                        ...
                        <TouchableOpacity style={activeButton.button} onPress={() => navigation.navigate('Checkout')}>
                            <Text style={activeButton.text}>Checkout</Text>
                        </TouchableOpacity>
                    </ScrollView>
                    :
                    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', marginBottom: hp('15%') }}>
                        <CartIcon color={BKColor.textColor2} width={wp('30%')} height={wp('30%')} />
                        <Text style={[styles.totalItemHeading, { fontWeight: '700' }]}>Your cart is empty</Text>
                        <Text style={[styles.totalItemHeading, { textAlign: 'center' }]}>Looks like you haven't added anything to your cart yet</Text>
                    </View>
                }
